...
#
# Supported locales on FreeCAD <2024-11-20, FreeCADGui.supportedLocales(), total=44>:
...
update_locale() {
	...
	if [ "$u" == "" ]; then
		eval $LUPDATE ../../InitGui.py -ts "${WB}.ts" # locale-agnostic file
	else
		eval $LUPDATE ../../InitGui.py -source-language en_US -target-language "${locale//-/_}" \
			-ts "${WB}_${locale}.ts"
	fi
}
